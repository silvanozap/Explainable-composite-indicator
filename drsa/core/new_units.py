...
Variable layout:
  [rho_al(n_al) | rho_am(n_am) | s_minus(x1),s_plus(x1),...,s_minus(xK),s_plus(xK) | eta(2*n_A)]
...
def classify_new_units(new_units, mat_sm, mat_sp,
    ...
    # Step 1: eq (4) with maximal rules
    cl_new = np.array([
    ...
    # Build base coverage constraints (minimal_set_rules WITHOUT rho bounds)
    d_al_vec = mat_sm[:, -1]
    d_am_vec = mat_sp[:, -1]
    ...
    for i in range(n_A):
        if d_al_vec[i] > 1:
            tt  = (al_rules_max[:, -1] == d_al_vec[i]).astype(float)
            row = np.zeros(n_al + n_am)
            row[:n_al] = -tt * match_al[i, :]
            rows_base.append(row); b_base.append(-1.0)
    for i in range(n_A):
        if d_am_vec[i] < p:
            tt  = (am_rules_max[:, -1] == d_am_vec[i]).astype(float)
            row = np.zeros(n_al + n_am)
            row[n_al:] = -tt * match_am[i, :]
            rows_base.append(row); b_base.append(-1.0)

    A_base     = np.array(rows_base, dtype=float) if rows_base else np.zeros((0, n_al+n_am))
    b_base_arr = np.array(b_base,    dtype=float) if b_base    else np.zeros(0)
    n_base     = A_base.shape[0]   # number of coverage constraint rows

    pos_start = n_al + n_am   # start of s variables

    # Extend base with s columns
    A6 = np.hstack([A_base, np.zeros((n_base, 2 * n_new))])
    b6 = b_base_arr.copy()

    # Add new unit constraints
    d = 0
    for k in range(n_new):
        ...
        if np.sum(test_am) == 0:
            ...
            V = np.zeros(nc); V[n_al:n_al+n_am] = -test_am
            A6 = np.vstack([A6, V]); b6 = np.append(b6, -1.0)
            for i in range(n_am):
                if test_am[i] == 1:
                    V = np.zeros(nc)
    ...
    # Non-contradictory: s_minus(xk) - s_plus(xk) <= 0
    neg_sigma = -_createsigma(n_new)
    block = np.hstack([np.zeros((n_new, n_al+n_am)), neg_sigma])
    A6 = np.vstack([A6, block]); b6 = np.append(b6, np.zeros(n_new))

    # Add eta columns: -I on first n_base rows (coverage constraints), zeros elsewhere
    posizione  = A6.shape[1]   # = n_al + n_am + 2*n_new
    n_rows_now = A6.shape[0]
    eta_cols   = np.zeros((n_rows_now, 2 * n_A))
    eta_cols[:n_base, :] = -np.eye(2 * n_A)[:n_base, :]
    ...
    lb6 = np.zeros(n_vars6); ub6 = np.ones(n_vars6)
    for k in range(n_new):
    ...
    res6 = milp(f6, constraints=constraints6, integrality=integrality6, bounds=bounds6)
    ...
    f7 = np.hstack([-np.ones(n_al + n_am), np.zeros(n_vars6 - n_al - n_am)])
    n_eta = 2 * n_A
    ...
    res7 = milp(f7, constraints=constraints7, integrality=integrality6, bounds=bounds6)
    ...
    # Find changed units (main.m line 132)
    changed = np.where(
        ~np.all(cl_A7 == np.column_stack([mat_sm[:,-1], mat_sp[:,-1]]), axis=1)
    )[0].tolist()
    result['changed_units'] = changed

    # Update s_plus for changed units (main.m line 135)
    ...
    if n_al7 + n_am7 > 0:
        n_A8 = len(all_sm8)
        d_al8 = all_sm8[:,-1]; d_am8 = all_sp8[:,-1]
        rows8, b8 = [], []
        for i in range(n_A8):
            if d_al8[i] > 1:
                tt = (al_max7[:,-1] == d_al8[i]).astype(float)
                row = np.zeros(n_al7+n_am7); row[:n_al7] = -tt * all_mal8[i,:]
                rows8.append(row); b8.append(-1.0)
        for i in range(n_A8):
            if d_am8[i] < p:
                tt = (am_max7[:,-1] == d_am8[i]).astype(float)
                row = np.zeros(n_al7+n_am7); row[n_al7:] = -tt * all_mam8[i,:]
                rows8.append(row); b8.append(-1.0)
        A8 = np.array(rows8) if rows8 else np.zeros((1,n_al7+n_am7))
        b8 = np.array(b8)    if b8    else np.zeros(1)
        n8 = n_al7+n_am7
        res8 = milp(np.ones(n8),
                    constraints=LinearConstraint(A8, -np.inf, b8),
        ...
        if res8.success:
            ...
            result.update({'step8_al_rules': al_min8, 'step8_am_rules': am_min8,
                           'milp_success': True,
                           'milp_message': f"Minimal: {len(al_min8)} at-least, {len(am_min8)} at-most"})
        else:
            result.update({'step8_al_rules': al_max7, 'step8_am_rules': am_max7,
                           'milp_success': False,
                           'milp_message': f"MILP (8) failed: {res8.message}"})
    else:
        result.update({'step8_al_rules': np.empty((0,)), 'step8_am_rules': np.empty((0,)),
                       'milp_success': True, 'milp_message': "No rules"})
